...
class IllegalCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "API,Illegal Character? Oh no!", details
        )
...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "API, Expected Character? ", details
        )
...
class InvalidSyntaxError(Error):
    def __init__(self, pos_start, pos_end, details=""):
        super().__init__(
            pos_start, pos_end, "API what's this? Invailed Syntax?", details
        )
...
class RTError(Error):
    def __init__(self, pos_start, pos_end, details, context):
        super().__init__(
            pos_start, pos_end, "API, what's this? Runtime Error?", details
        )
...
class Parser:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'return', 'continue', 'break', 'let', 'if', 'for', 'while', 'api', int, float, identifier, '+', '-', '(', '[' or 'not'",
                )
            )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expeted 'let', 'if', 'for', 'while', 'api', int, float, identifier, '+', '-', '(', '[' or 'not'",
                )
            )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'api' or 'not'",
                )
            )
                ...
                if res.error:
                    return res.failure(
                        InvalidSyntaxError(
                            self.current_tok.pos_start,
                            self.current_tok.pos_end,
                            "Expected ')', 'let', 'if', 'for', 'while', 'api', int, float, identifier, '+', '-', '(', '[' or 'not'",
                        )
                    )
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, "api"):
        ...
        return res.failure(
            InvalidSyntaxError(
                tok.pos_start,
                tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', if', 'for', 'while', 'api'",
            )
        )
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, "api"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
        ...
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(
                    other.pos_start,
                    other.pos_end,
                    "Element at this index could not be retrieved from the list because the index is out of bound",
                    self.context,
                )
        else:
            return None, Value.illegal_operation(self, other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_input_int(self, exec_ctx):
        while True:
            ...
            try:
                ...
                print(f"'{text}' must be an integer. >w<")
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "Element at this index could not be removed from list because index is owt of bounds",
                    exec_ctx,
                )
            )
    ...
    def execute_len(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            return RTResult().failure(
                RTError(
                    self.pos_start, self.pos_end, "Argument must be list", exec_ctx
                )
            )
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f'TwT Failed to load script "{fn}"\n' + str(e),
                    exec_ctx,
                )
            )
        ...
        if error:
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    f'TwT Failed to finish executing script "{fn}"\n'
                    + error.as_string(),
                    exec_ctx,
                )
            )
...
global_symbol_table.set("impress", BuiltInFunction.print)
global_symbol_table.set("impress_ret", BuiltInFunction.print_ret)
global_symbol_table.set("display", BuiltInFunction.print)
global_symbol_table.set("display_ret", BuiltInFunction.print_ret)
global_symbol_table.set("scanf", BuiltInFunction.input)
global_symbol_table.set("scanf_int", BuiltInFunction.input_int)
